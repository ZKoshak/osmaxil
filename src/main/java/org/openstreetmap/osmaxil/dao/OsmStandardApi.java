...
public class OsmStandardApi {
	...
	public OsmXmlRoot readElement(long id, ElementType type) {
		...
		try {
			...
			ResponseEntity<OsmXmlRoot> responseEntity = this.restTemplate.exchange(this.url + type.getName() + "/" + id, HttpMethod.GET,
					requestEntity, OsmXmlRoot.class);
	...
	private long createChangeset() throws RestClientException {
		...
		try {
			...
			ResponseEntity<String> responseEntity = this.restTemplate.exchange(this.url + "changeset/create", HttpMethod.PUT, requestEntity,
					String.class);
